...
#OPTIONS
 ...
 #8.  Percentage of Islands that are not removed (as decimal)
     islands.to.be.removed=0.8  #WARNING!!!!  TEMP. CHANGED,  may be better at .5######## 
...
#setwd(Save.location)
xkcd=1;  Save.loc2.test=paste(Save.loc2,xkcd)
while (file.exists(Save.loc2.test)==T){
      xkcd=xkcd+1
      Save.loc2.test=paste(Save.loc2,xkcd)      
}
setwd(dir.create(Save.loc2.test))
...
for(j in 1:length(alpha))  {
      ...
      windows()
      plot(ss$prop.area,ss$eigen.patch.rm,main=(paste("Alpha=",alpha[j])))
      points(ss$prop.area,ss$eigen.islands,col="red")
      ...
      for (i in 1:num.breaks){
          temp.di=subset(ss$eigen.islands,ss$prop.area>=(1-islands.to.be.removed+(br.s*(i-1)))&ss$prop.area<(1-islands.to.be.removed+(br.s*i)))
          eigen.islands.subset[i,1:length(temp.di)]=temp.di
          temp.di=subset(ss$eigen.quality,ss$prop.area>=(1-islands.to.be.removed+(br.s*(i-1)))&ss$prop.area<(1-islands.to.be.removed+(br.s*i)))
          eigen.quality.subset[i,1:length(temp.di)]=temp.di
          temp.di=subset(ss$eigen.patch.rm,ss$prop.area>=(1-islands.to.be.removed+(br.s*(i-1)))&ss$prop.area<(1-islands.to.be.removed+(br.s*i)))
