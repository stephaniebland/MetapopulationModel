...
landscape.summary<-data.frame(bunch.of.repeats, min.dist, second.dist, var.dist, mean.dist, num, Tx, Ty, size.dist, spatial.dist, mu); landscape.summary.name<-paste("landscape summary", unique.lab,"-",bunch.of.repeats,".txt",sep="")
...
#IS THIS NECESSARY??? 
#tmp.name<-paste("Summary",unique.lab,"-",bunch.of.repeats,".txt",sep="")
...
write.table(out.data,file=tmp.name,sep="\t",row.names=T)
...
#TEST
tues=cbind(prop.area,t.undisturbed.eigen,eigen.patch.rm,eigen.islands,eigen.quality,num.patch.rem,t.alpha.count,t.bunch.of.repeats,mean.nn,var.nn)
summary(tues)                     