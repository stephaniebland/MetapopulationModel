...
###########I don't know what trials I should run.
#  The display will not work for large data sets.  I think the most important part is to get 
#  replicates for each landscape.  So that means repeats, islands.to.be.removed, and alpha.
#  But then again, we want to change the layout of patches.  Ex:  spatial.dist & clustering
...
#OPTIONS
 ...
 #8.  Percentage of Islands that are not removed (as decimal)
     islands.to.be.removed=0.99  #WARNING!!!!  TEMP. CHANGED,  may be better at .5######## 
 #9.  Repeats for habitat destruction
      #repeats is the number of time we repeat simulation  
     repeats=10  #WARNING!!!!  TEMP. CHANGED, may be better at 100########
 #10. Change Alpha (for natural landscape - alpha changes according to how much land you remove)
     alpha=seq(.05,.5,0.05)#c(seq(.05,3,0.05),seq(3,20,1))
 #11. Change mu
     mu=1
 #12. Total number of repeats for the entire program
     number.of.bunch.of.repeats=1
 #13. Where to save the file?    
     #Save.location="M:/R Program/DATA3"
     Save.loc2<-"C:/Users/Stephie!!!/Dropbox/R Program" #"M:/R Program"   #  This will create a new folder in this location called DATA-(program run number)
     #Save.location="C:/Users/Ben/Dropbox/R code/R code/habitat loss versus degradation metapopulation"
     save.as="pdf"    #Others are "bmp", "png", "tiff"
...
#overall data output
#if (file.exists(Save.location)==F) {file.create(Save.location)}
#setwd(Save.location)
xkcd=1;  Save.loc2.test=paste(Save.loc2,"/DATA-",xkcd,sep="")
while (file.exists(Save.loc2.test)==T){
      xkcd=xkcd+1
      Save.loc2.test=paste(Save.loc2,"/DATA-",xkcd,sep="")      
}
dir.create(Save.loc2.test);setwd(Save.loc2.test)
...
#DISTANCE METRICS!
        ...
        plot.name0<-paste("landscape summary", unique.lab,"-",bunch.of.repeats,sep="")
        savePlot(filename=paste(plot.name0,".",save.as,sep=""), type=c(save.as))
...
write.table(nice.su.stats,file=(paste("nice",tmp.name)),sep="\t",row.names=T)
...
v=length(alpha)
j.leg=v*number.of.bunch.of.repeats; for(j in 0:(j.leg-1))  {
      LOTR=paste("rep=",(j+v-j%%v)/v,sep="")
      #ss<-subset(nice.su.stats,alpha.count==alpha[j]); inc.rep=j; LOTR="all.reps"
      ss<-subset(nice.su.stats,alpha.count==alpha[1+j%%v]&bunch.of.repeats==(j+v-j%%v)/v); inc.rep=1+j%%v
      print(paste(alpha[1+j%%v],", ",(j+v-j%%v)/v))
      scaling=max(ss$eigen.islands,ss$eigen.quality,ss$eigen.patch.rm,na.rm=T)
      #min.scale=min(ss$eigen.islands,ss$eigen.quality,ss$eigen.patch.rm,na.rm=T)   #Incase necessary
      min.scale=min(nice.su.stats$eigen.islands,nice.su.stats$eigen.quality,nice.su.stats$eigen.patch.rm,na.rm=T)
 
      windows()
      plot(ss$prop.area,ss$eigen.patch.rm/scaling,main=(paste("Alpha=",alpha[inc.rep])),xlab="Proportion Area Lost",ylab="Propotional Eigenvalue",ylim=(c(0,max(ss$eigen.islands,ss$eigen.quality,ss$eigen.patch.rm,na.rm=T)))/scaling)
      points(ss$prop.area,ss$eigen.islands/scaling,col="red")
      points(ss$prop.area,ss$eigen.quality/scaling,col="blue")
      hobbes=filter(ss$eigen.patch.rm[order(ss$prop.area)],rep(1/51,51),sides=2)
      lines(sort(ss$prop.area),hobbes/scaling,col="grey")
      lines(smooth.spline(ss$prop.area,ss$eigen.patch.rm/scaling,spar=1.5),col="midnightblue" )
      plot.name1=paste("Point-plot(prop.area),alpha=",alpha[inc.rep],",",LOTR,",",unique.lab,sep="")
      savePlot(filename=paste(plot.name1,".",save.as,sep=""), type=c(save.as))
      ...
      num.breaks=round((1-min(ss$prop.area)+min(ss$prop.area)%%br.s)/br.s)
      eigen.islands.subset=rep(NA,num.breaks*repeats*(br.s*num.breaks*num)*number.of.bunch.of.repeats);dim(eigen.islands.subset)=c(num.breaks,repeats*(br.s*num.breaks*num)*number.of.bunch.of.repeats)
      eigen.quality.subset=rep(NA,num.breaks*repeats*(br.s*num.breaks*num)*number.of.bunch.of.repeats);dim(eigen.quality.subset)=c(num.breaks,repeats*(br.s*num.breaks*num)*number.of.bunch.of.repeats)
      eigen.patch.rm.subset=rep(NA,num.breaks*repeats*(br.s*num.breaks*num)*number.of.bunch.of.repeats);dim(eigen.patch.rm.subset)=c(num.breaks,repeats*(br.s*num.breaks*num)*number.of.bunch.of.repeats)
      label.job=rep(NA,num.breaks);dim(label.job)=num.breaks
      for (i in 1:num.breaks){
          temp.di=subset(ss$eigen.islands,ss$prop.area>=(1-br.s*num.breaks+(br.s*(i-1)))&ss$prop.area<(1-br.s*num.breaks+(br.s*i)))
          if (length(temp.di)!=0){eigen.islands.subset[i,1:length(temp.di)]=temp.di}
          temp.di=subset(ss$eigen.quality,ss$prop.area>=(1-br.s*num.breaks+(br.s*(i-1)))&ss$prop.area<(1-br.s*num.breaks+(br.s*i)))
          if (length(temp.di)!=0){eigen.quality.subset[i,1:length(temp.di)]=temp.di}
          temp.di=subset(ss$eigen.patch.rm,ss$prop.area>=(1-br.s*num.breaks+(br.s*(i-1)))&ss$prop.area<(1-br.s*num.breaks+(br.s*i)))
          if (length(temp.di)!=0){eigen.patch.rm.subset[i,1:length(temp.di)]=temp.di}
          label.job[i]=paste((1-br.s*num.breaks+(br.s*(i-1))),"-",(1-br.s*num.breaks+(br.s*i)))
      }
      scaling2=max(eigen.islands.subset,eigen.quality.subset,eigen.patch.rm.subset,na.rm=T)
      boxplot(t(eigen.patch.rm.subset)/scaling2,names=label.job,xlab="Proportion Area Lost",ylab="Propotional Eigenvalue", main=(paste("Alpha=",alpha[inc.rep])),ylim=c(0,1)); boxplot(t(eigen.quality.subset)/scaling2,col="blue",add=T,names=label.job); boxplot(t(eigen.islands.subset)/scaling2,col="red",add=T,names=label.job)
      plot.name2=paste("Barplot,alpha=",alpha[inc.rep],",",LOTR,",",unique.lab,sep="")
      savePlot(filename=paste(plot.name2,".",save.as,sep=""), type=c(save.as))
